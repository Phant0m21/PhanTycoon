...
TICKET_PANEL_CHANNEL_ID = 1532969580605997258
TICKET_PANEL_MESSAGE_ID = 1532983983439351901
TICKET_CATEGORY_ID = 1532458676122550333
TICKET_LOG_CHANNEL_ID = 1532977518699610202
TICKET_STAFF_ROLE_IDS = {
    1532460335775224000, 1532460411243462898,
    1532460794141475007, 1532460593154490388,
}
MAX_OPEN_TICKETS = 3
...
class CloseTicketModal(disnake.ui.Modal):
    ...
    async def callback(self, inter):
        ...
        embed.add_field(name="Type", value=TICKET_TYPES[ticket["ticket_type"]]["label"], inline=True)
        embed.add_field(name="Opened by", value=f"<@{ticket['owner_id']}> (`{ticket['owner_id']}`)", inline=True)
        embed.add_field(name="Closed by", value=f"{inter.author.mention} (`{inter.author.id}`)", inline=True)
        embed.add_field(name="Open for", value=str(duration).split(".")[0], inline=True)
        embed.add_field(name="Messages saved", value=str(len(messages)), inline=True)
